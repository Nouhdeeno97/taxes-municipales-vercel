  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card className="border-blue-100"><CardHeader><Badge className="w-fit bg-blue-700">Étape 3</Badge><CardTitle className="mt-3">Affecter la règle à une activité</CardTitle><CardDescription>L’affectation relie la taxe à un redevable et à une activité réelle. Une activité peut porter plusieurs taxes.</CardDescription></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-[1fr,1fr,auto] md:items-end" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const input = { activityId: String(data.get("activityId")), taxRuleId: String(data.get("taxRuleId")), startDate: new Date(String(data.get("startDate"))) }; if (!offline.online) { offline.defer("ASSIGN_RULE", input, "Affectation de règle fiscale"); toast.success("Affectation enregistrée localement et en attente de synchronisation."); event.currentTarget.reset(); return; } assignRule.mutate(input); }}><div><Label>Activité</Label><select name="activityId" className={selectStyle} required><option value="">Sélectionner</option>{activities.data?.map(({ activity }) => <option key={activity.id} value={activity.id}>{activity.reference} · {activity.label}</option>)}</select></div><div><Label>Règle fiscale active</Label><select name="taxRuleId" className={selectStyle} required><option value="">Sélectionner</option>{rules.data?.filter(({ rule }) => rule.isActive).map(({ rule, taxType }) => <option key={rule.id} value={rule.id}>{rule.label} · {money.format(Number(rule.baseAmount))} XAF · {taxType.label}</option>)}</select></div><div><Label>Début</Label><Input name="startDate" type="date" defaultValue={today()} className="mt-1" required /><Button type="submit" disabled={assignRule.isPending} className="mt-2 w-full bg-blue-700 hover:bg-blue-800"><Link2 className="mr-2 size-4" />Affecter</Button></div></form></CardContent></Card>
        <Card className="border-blue-100"><CardHeader><Badge className="w-fit bg-blue-700">Étape 4</Badge><CardTitle className="mt-3">Générer les obligations</CardTitle><CardDescription>Choisissez l’activité et la période. L’opération est idempotente : la même règle et la même période ne créent pas de doublon.</CardDescription></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-[1fr,1fr,1fr]" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const input = { activityId: String(data.get("activityId")), periodStart: new Date(String(data.get("periodStart"))), periodEnd: new Date(String(data.get("periodEnd"))), dueDate: new Date(String(data.get("dueDate"))) }; if (!offline.online) { offline.defer("GENERATE_OBLIGATIONS", input, "Génération d’obligations"); toast.success("Génération enregistrée localement et en attente de synchronisation."); event.currentTarget.reset(); return; } generate.mutate(input); }}><div className="md:col-span-3"><Label>Activité à générer</Label><select name="activityId" className={selectStyle} required><option value="">Sélectionner</option>{activities.data?.map(({ activity }) => <option key={activity.id} value={activity.id}>{activity.reference} · {activity.label}</option>)}</select></div><div><Label>Début de période</Label><Input name="periodStart" type="date" defaultValue={today()} className="mt-1" required /></div><div><Label>Fin de période</Label><Input name="periodEnd" type="date" defaultValue={today()} className="mt-1" required /></div><div><Label>Échéance</Label><Input name="dueDate" type="date" defaultValue={today()} className="mt-1" required /></div><div className="md:col-span-3"><Button type="submit" disabled={generate.isPending} className="bg-blue-700 hover:bg-blue-800"><Calculator className="mr-2 size-4" />Générer les obligations</Button></div></form></CardContent></Card>
      </div>
