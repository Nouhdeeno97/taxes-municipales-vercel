  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <Card className="border-blue-100">
          ...
          <CardContent className="space-y-4">
            <form className="space-y-2 rounded-xl bg-slate-50 p-3" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); createTaxCategory.mutate({ code: String(data.get("code")), label: String(data.get("label")) }); event.currentTarget.reset(); }}>
            ...
            <form className="space-y-2 rounded-xl bg-slate-50 p-3" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); createTaxType.mutate({ categoryId: String(data.get("categoryId")) || undefined, code: String(data.get("code")), label: String(data.get("label")) }); event.currentTarget.reset(); }}>
              <p className="text-sm font-semibold">Nouveau type de taxe</p>
              <select name="categoryId" className={selectStyle}><option value="">Sans catégorie</option>{catalog.data?.taxCategories.map(category => <option key={category.id} value={category.id}>{category.label}</option>)}</select>
            ...
            <form className="space-y-2 rounded-xl bg-slate-50 p-3" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); createPeriodicity.mutate({ code: String(data.get("code")), label: String(data.get("label")), calendarUnit: String(data.get("unit")) as "DAY" | "WEEK" | "MONTH" | "QUARTER" | "SEMESTER" | "YEAR" | "CUSTOM", intervalCount: 1 }); event.currentTarget.reset(); }}>
        ...
        <Card className="border-blue-100 xl:col-span-2">
          ...
          <CardContent>
            <form className="grid gap-4 md:grid-cols-2" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); createRule.mutate({ taxTypeId: String(data.get("taxTypeId")), periodicityId: String(data.get("periodicityId")), code: String(data.get("code")), label: String(data.get("label")), baseAmount: Number(data.get("baseAmount")), graceDays: Number(data.get("graceDays") || 0), penaltyRate: Number(data.get("penaltyRate") || 0) / 100, validFrom: new Date(String(data.get("validFrom"))) }); event.currentTarget.reset(); }}>
              <div><Label>Type de taxe actif</Label><select name="taxTypeId" className={selectStyle} required><option value="">Sélectionner</option>{activeTaxTypes.map(type => <option key={type.id} value={type.id}>{type.label}</option>)}</select></div>
              <div><Label>Périodicité</Label><select name="periodicityId" className={selectStyle} required><option value="">Sélectionner</option>{catalog.data?.periodicities.filter(item => item.isActive).map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Card className="border-blue-100"><CardHeader><Badge className="w-fit bg-blue-700">Étape 3</Badge><CardTitle className="mt-3">Affecter la règle à une activité</CardTitle><CardDescription>L’affectation relie la taxe à un redevable et à une activité réelle. Une activité peut porter plusieurs taxes.</CardDescription></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-[1fr,1fr,auto] md:items-end" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); assignRule.mutate({ activityId: String(data.get("activityId")), taxRuleId: String(data.get("taxRuleId")), startDate: new Date(String(data.get("startDate"))) }); }}><div><Label>Activité</Label><select name="activityId" className={selectStyle} required><option value="">Sélectionner</option>{activities.data?.map(({ activity }) => <option key={activity.id} value={activity.id}>{activity.reference} · {activity.label}</option>)}</select></div><div><Label>Règle fiscale active</Label><select name="taxRuleId" className={selectStyle} required><option value="">Sélectionner</option>{rules.data?.filter(({ rule }) => rule.isActive).map(({ rule, taxType }) => <option key={rule.id} value={rule.id}>{rule.label} · {money.format(Number(rule.baseAmount))} XAF · {taxType.label}</option>)}</select></div><div><Label>Début</Label><Input name="startDate" type="date" defaultValue={today()} className="mt-1" required /><Button type="submit" disabled={assignRule.isPending} className="mt-2 w-full bg-blue-700 hover:bg-blue-800"><Link2 className="mr-2 size-4" />Affecter</Button></div></form></CardContent></Card>
