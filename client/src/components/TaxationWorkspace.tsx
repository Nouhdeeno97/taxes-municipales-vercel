...
export function TaxationWorkspace() {
  ...
  const [obligationPage, setObligationPage] = useState(0);
  const obligationsPage = trpc.municipal.taxation.obligationsPage.useQuery({ page: obligationPage, pageSize: 10, search: obligationSearch || undefined });
  ...
  const refreshCatalog = () => {
    ...
    utils.municipal.taxation.obligationsPage.invalidate();
  };
      ...
      <Card className="border-blue-100">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-slate-200 p-4"><p className="font-semibold text-slate-950">Lots par type d’activité</p><div className="mt-3 grid max-h-[11.5rem] grid-cols-2 gap-2 overflow-y-auto pr-1">{activityLots.data?.activityTypes.map(type => <label key={type.id} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm text-slate-700"><input type="checkbox" checked={selectedActivityTypeIds.includes(type.id)} onChange={() => toggleSelection(type.id, selectedActivityTypeIds, setSelectedActivityTypeIds)} className="size-4 shrink-0 accent-blue-700" /><span className="min-w-0 truncate" title={type.label}>{type.label} <strong className="text-slate-500">({type.count})</strong></span></label>) ?? <p className="col-span-2 text-sm text-slate-500">Aucun type avec activité active.</p>}</div></div>
            <div className="rounded-xl border border-slate-200 p-4"><p className="font-semibold text-slate-950">Lots par type de localisation</p><div className="mt-3 grid max-h-[11.5rem] grid-cols-2 gap-2 overflow-y-auto pr-1">{activityLots.data?.locationTypes.map(item => <label key={item.locationType} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm text-slate-700"><input type="checkbox" checked={selectedActivityLocationTypes.includes(item.locationType)} onChange={() => toggleSelection(item.locationType, selectedActivityLocationTypes, setSelectedActivityLocationTypes)} className="size-4 shrink-0 accent-blue-700" /><span className="min-w-0 truncate" title={locationTypeLabels[item.locationType] ?? item.locationType}>{locationTypeLabels[item.locationType] ?? item.locationType} <strong className="text-slate-500">({item.count})</strong></span></label>) ?? <p className="col-span-2 text-sm text-slate-500">Aucun type de localisation disponible.</p>}</div></div>
          </div>
